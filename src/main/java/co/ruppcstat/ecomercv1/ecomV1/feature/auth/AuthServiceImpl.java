...
//        }
    ...
    @Override
    public void updateRoll(String email, UpdateRoleCreate updateRoleCreate) {
        User user=userRepository.findByEmail(email)
                .orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"Email not found"));
//        List<String> keyIdList = updateRoleCreate.roleName();
//        List<Role> roleList = new java.util.ArrayList<>(List.of());
//
//        for (int i = 0;i<keyIdList.size();i++) {
//            System.out.println(keyIdList.get(i));
////            Role role = roleRepository.findByRoleUser()
////                    .orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"product codeProduct not found"));
//
//           // productList.add(role);
//            System.out.println("is not work");
//        }

        userMapper.updateRoll(user,updateRoleCreate);
        userRepository.save(user);
    }
